...
{	@Override
	...
	@Select("<script>"
		...
		+ "</when>"
		+ "<otherwise>"
		+ "SELECT TAID,TAASID,TASI,TAPI,TAHZ,TAST,TAUSID,TACRDD,TACRDT,TAUPDD,TAUPDT FROM public.F1131 WHERE TAID=#{taid}"
		+ "</otherwise>"
		+ "</choose>"
		+ "</script>")
	public Segmentflow get(@Param("taid")Integer taid,@Param("mask")Mask mask);
	...
	@Update("<script>"
		...
		+ "</when>"
		+ "<otherwise>"
		+ "UPDATE public.F1131 SET TAASID=#{bean.taasid},TASI=#{bean.tasi},TAPI=#{bean.tapi},TAHZ=#{bean.tahz},TAST=#{bean.tast},TAUSID=#{bean.tausid},TACRDD=#{bean.tacrdd},TACRDT=#{bean.tacrdt},TAUPDD=#{bean.taupdd},TAUPDT=#{bean.taupdt} WHERE TAID=#{bean.taid}"
		+ "</otherwise>"
		+ "</choose>"
		+ "</script>")
	public void update(@Param("bean")Segmentflow bean,@Param("mask")Mask mask);
	...
	@Select("<script>"
		...
		+ "</when>"
		+ "<otherwise>"
		+ "SELECT TAID,TAASID,TASI,TAPI,TAHZ,TAST,TAUSID,TACRDD,TACRDT,TAUPDD,TAUPDT "
		+ "</otherwise>"
		+ "</choose>"
		+ "FROM public.F1131 "
		...
		+ "</script>")
	public List<Segmentflow> query(@Param("filter")String filter,@Param("order")String order,@Param("start")long start,@Param("count")long count,@Param("mask")Mask mask);
	...
	@Select("SELECT COALESCE(MAX(tasi),0)+1 FROM public.F1131 WHERE taasid=#{taasid}")
