...
{	@Override
	...
	@Override
	@Select("SELECT TRASID,TRPI,TRHZ,TRST,TRUSID,TRUPDD,TRUPDT FROM public.F1130 WHERE TRASID=#{trasid}")
	public Segment get(@Param("trasid")Integer trasid);
	...
	@Select("<script>"
		...
		+ "</if>"
		+ "</script>")
	public Segment getWithMask(@Param("trasid")Integer trasid,@Param("mask")Mask mask);
	...
	@Override
	@Update("UPDATE public.F1130 SET TRPI=#{bean.trpi},TRHZ=#{bean.trhz},TRST=#{bean.trst},TRUSID=#{bean.trusid},TRUPDD=#{bean.trupdd},TRUPDT=#{bean.trupdt} WHERE TRASID=#{bean.trasid}")
	public void update(@Param("bean")Segment bean);
	...
	@Update("<script>"
		...
		+ "</if>"
		+ "</script>")
	public void updateWithMask(@Param("bean")Segment bean,@Param("mask")Mask mask);
	...
	@Override
	@Select("<script>"
		+ "SELECT TRASID,TRPI,TRHZ,TRST,TRUSID,TRUPDD,TRUPDT FROM public.F1130 "
		+ "<if test='filter!=null'>WHERE ${filter} </if>"
		+ "<if test='order!=null'>ORDER BY ${order} </if>"
		+ "LIMIT #{count} OFFSET #{start}"
		+ "</script>")
	public List<Segment> query(@Param("filter")String filter,@Param("order")String order,@Param("start")long start,@Param("count")long count);
	...
	@Select("<script>"
		...
		+ "LIMIT #{count} OFFSET #{start}"
		+ "</script>")
	public List<Segment> queryWithMask(@Param("filter")String filter,@Param("order")String order,@Param("start")long start,@Param("count")long count,@Param("mask")Mask mask);
