...
{	@Override
	...
	@Override
	@Select("SELECT LSID,LSNAME,LSLOC,LSSORT FROM public.F1010 WHERE LSID=#{lsid}")
	public Language get(@Param("lsid")String lsid);
	...
	@Select("<script>"
		...
		+ "</if>"
		+ "</script>")
	public Language getWithMask(@Param("lsid")String lsid,@Param("mask")Mask mask);
	...
	@Override
	@Update("UPDATE public.F1010 SET LSNAME=#{bean.lsname},LSLOC=#{bean.lsloc},LSSORT=#{bean.lssort} WHERE LSID=#{bean.lsid}")
	public void update(@Param("bean")Language bean);
	...
	@Update("<script>"
		...
		+ "</if>"
		+ "</script>")
	public void updateWithMask(@Param("bean")Language bean,@Param("mask")Mask mask);
	...
	@Override
	@Select("<script>"
		+ "SELECT LSID,LSNAME,LSLOC,LSSORT FROM public.F1010 "
		+ "<if test='filter!=null'>WHERE ${filter} </if>"
		+ "<if test='order!=null'>ORDER BY ${order} </if>"
		+ "LIMIT #{count} OFFSET #{start}"
		+ "</script>")
	public List<Language> query(@Param("filter")String filter,@Param("order")String order,@Param("start")long start,@Param("count")long count);
	...
	@Select("<script>"
		...
		+ "LIMIT #{count} OFFSET #{start}"
		+ "</script>")
	public List<Language> queryWithMask(@Param("filter")String filter,@Param("order")String order,@Param("start")long start,@Param("count")long count,@Param("mask")Mask mask);
