...
@RestController
@RequestMapping(value="/api/authority_check",produces="application/json;charset=UTF-8")
public class AuthorityCheckController extends RestCheckControllerBase<Authority,String>
{	@RequestMapping(value="/{auid}",method=RequestMethod.POST)
	...
	{	if(result.hasErrors())
		...
		if(((AuthorityMapper)bMapper).exists(bean.getAuid())||!((AuthorityMapper)bMapper).unique(bean))
	...
	private List<List<String>> referencing(Authority bean)
	{	List<List<String>> fields=new ArrayList<List<String>>();
		return fields;
	}
	private List<List<String>> referencedAndchanged(Authority origBean,Authority bean)
	{	List<List<String>> fields=new ArrayList<List<String>>();
		if(!origBean.getAuid().equals(bean.getAuid()))
			if(((AuthorityMapper)bMapper).referencedAuid(origBean.getAuid()))
				fields.add(java.util.Arrays.asList("auid"));
		return fields;
	}
	private List<List<String>> referenced(Authority origBean)
	{	List<List<String>> fields=new ArrayList<List<String>>();
		if(((AuthorityMapper)bMapper).referencedAuid(origBean.getAuid()))
			fields.add(java.util.Arrays.asList("auid"));
		return fields;
