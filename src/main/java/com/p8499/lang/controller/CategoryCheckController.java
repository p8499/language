...
@RestController
@RequestMapping(value="/api/category_check",produces="application/json;charset=UTF-8")
public class CategoryCheckController extends RestCheckControllerBase<Category,Integer>
{	@RequestMapping(method=RequestMethod.POST)
	...
	private List<List<String>> referencing(Category bean)
	{	List<List<String>> fields=new ArrayList<List<String>>();
		if(!((CategoryMapper)bMapper).referencingCglsid(bean.getCglsid()))
		{	fields.add(java.util.Arrays.asList("cglsid"));
		}
		return fields;
	}
	private List<List<String>> referencedAndchanged(Category origBean,Category bean)
	{	List<List<String>> fields=new ArrayList<List<String>>();
		if(!origBean.getCgid().equals(bean.getCgid()))
			if(((CategoryMapper)bMapper).referencedCgid(origBean.getCgid()))
				fields.add(java.util.Arrays.asList("cgid"));
		return fields;
	}
	private List<List<String>> referenced(Category origBean)
	{	List<List<String>> fields=new ArrayList<List<String>>();
		if(((CategoryMapper)bMapper).referencedCgid(origBean.getCgid()))
			fields.add(java.util.Arrays.asList("cgid"));
		return fields;
